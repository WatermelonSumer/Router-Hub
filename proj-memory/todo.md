...
- 实现榜单/详情/坟场只读接口 + 评价（充值 key 自证）。【已完成：/rank、详情页、坟场、C 端 user_topup 评价；真实站点额度接口兼容性待 VM 观察】
...
- 站点编辑/下架。【已完成：站长可编辑硬信息/探测身份；探测身份变更退回 pending；下架软删除】
- 中转集市对接流程状态机（超时/拒绝/取消）。
- 充值验证对 new-api/sub2api 不同版本额度接口的适配层。【已新增基础适配；真实站点差异待继续扩展】
